...
echo "Using files: $TEMPLATE_FILE, $PARAM_FILE, $OBSERVED_FILE, $BOOTSTRAP_FILE"
...
do
	echo "Doing bootstrap replicate $i/$ITERATIONS"
	mkdir $i
	...
	cd $i
    echo "$FSC_BIN -i $BOOTSTRAP_FILE -n 1 $FSC_FOLDING_FLAG 0 -c 2 -I $DO_MULTI &> /dev/null"
	$FSC_BIN -i $BOOTSTRAP_FILE -n 1 $FSC_FOLDING_FLAG 0 -c 2 -I $DO_MULTI &> /dev/null

	# I don't know wtf this is doing. Why run it twice? Delete the seed and 
	# the old bootstrap dir? Gotta ask Xander.
